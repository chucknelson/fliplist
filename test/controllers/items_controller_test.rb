...
class ItemsControllerTest < ActionController::TestCase
  ...
  test 'should create new item successfully' do
    ...
    expected_item_remaining_count =
      @list.items.where(completed: false).count + 1

    post :create,
         params: {
           user_id: @user.id,
           list_id: @list.id,
           item: { name: 'New Item', completed: false }
         }
  ...
  test 'should delete item successfully' do
    ...
    item = get_incomplete_item(@list)
    delete :destroy,
           params: { user_id: @user.id, list_id: @list.id, id: item.id }
    assert_response :redirect
