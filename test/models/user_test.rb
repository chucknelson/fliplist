...
class UserTest < ActiveSupport::TestCase
  ...
  test 'should not save user if email already exists in database' do
    ...
    assert_equal @test_user.email,
                 user.email,
                 'User email not set correctly (should match test user)'
    assert_not user.save,
               'User saved with an email that already exists in the database'
  end
  ...
  test 'should authenticate with a correct password (i.e., is bcrypt/has_secure_password working)' do
    ...
    assert user.authenticate('password'),
           'User did not authenticate as expected'
  end
end
