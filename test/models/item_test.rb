...
class ItemTest < ActiveSupport::TestCase
  def setup
    ...
    @new_item =
      Item.new(
        name: 'My Item', completed: false, sort_order: 9, list: @test_list
      )
  end
  ...
  test "should not update list's items remaining after item update failure" do
    item = @test_incomplete_item
    list = item.list
    assert_not item.update(completed: true, name: nil),
               'Item updated unexpectedly'
    list.reload
    assert_equal list.items.where(completed: false).size,
                 list.items_remaining,
                 'Items remaining updated unexpectedly'
  end
