...
class ListTest < ActiveSupport::TestCase
  ...
  test 'should be complete if no items are remaining' do
    ...
    assert list.items.where(completed: false).each { |item|
  ...
  test 'should update items remaining with number of incomplete items' do
    list = @test_list
    assert list.items.where(completed: false).size > 0,
           'Test list has no incomplete items'
    assert_equal list.items.where(completed: false).size,
                 list.items_remaining,
                 'Test list not set up correctly'
    ...
    assert_equal list.items_remaining,
                 list.items.where(completed: false).size,
                 'Items remaining out of sync with individual items'
    assert_equal expected_items_remaining,
                 list.items_remaining,
                 'Items remaining not updated correctly'
  end
end
